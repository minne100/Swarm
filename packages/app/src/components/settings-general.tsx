...
function isShellOption(value: unknown): value is ShellOption {
  if (!value || typeof value !== "object") return false
  const item = value as Record<string, unknown>
  return typeof item.path === "string" && typeof item.name === "string" && typeof item.acceptable === "boolean"
}

function normalizeShellOptions(value: unknown): ShellOption[] {
  if (Array.isArray(value)) return value.filter(isShellOption)
  if (!value || typeof value !== "object") return []
  const item = value as Record<string, unknown>
  if (Array.isArray(item.items)) return item.items.filter(isShellOption)
  if (Array.isArray(item.shells)) return item.shells.filter(isShellOption)
  return []
}
...
export const SettingsGeneral: Component = () => {
  ...
  const themeOptions = createMemo<ThemeOption[]>(() => {
    ...
    return ids.map((id) => ({ id, name: theme.name(id) }))
  })
  ...
  const [shells] = createResource(
    () =>
      globalSdk.client.pty
        .shells()
        .then((res) => normalizeShellOptions(res.data))
  ...
  const shellOptions = createMemo<ShellSelectOption[]>(() => {
    const list = normalizeShellOptions(shells.latest)
  ...
  const languageOptions = createMemo(() =>
    (Array.isArray(language.locales) ? language.locales : []).map((locale) => ({
      value: locale,
